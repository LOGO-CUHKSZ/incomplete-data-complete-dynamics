...
if __name__ == '__main__':
    ...
    for dataset in os.listdir(root):
        if dataset == 'temp':
            continue
        if not dataset.__contains__('block'):
            continue

        p = 0.01 * float(dataset.split('_')[-1].rstrip('.h5'))
        sampling_config['mask_func'] = partial(uniform_mask, p=p)
    ...
    # model_path = 'logs/ns_uniform_20/ldpv/karras/dm/ns_uniform_20---ldpv---karras---dm---data---seed42---2025_07_03__11_27_56/checkpoints/ckpt.pt'
    ...
    #     }
    # }
    # sampling_config['mask_func'] = partial(uniform_mask, p=0.01)

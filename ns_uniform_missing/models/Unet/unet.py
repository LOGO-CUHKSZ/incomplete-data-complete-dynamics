...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        return self.cnn(torch.cat((mask.float(), xt), 1), t)



class KarrasUnet3D(nn.Module):
    def __init__(
        self,
        hidden_size,
        input_channels = 2,
        ...
        **kwargs
        ...
        init_dim = default(init_dim, hidden_size)
        assert is_odd(init_kernel_size)

        init_padding = init_kernel_size // 2
        self.init_conv = nn.Conv3d(input_channels, init_dim, (1, init_kernel_size, init_kernel_size), padding = (0, init_padding, init_padding))

        self.init_temporal_attn = Residual(PreNorm(init_dim, temporal_attn(init_dim)))

        dims = [init_dim, *map(lambda m: hidden_size * m, dim_mults)]
        ...
        self.final_conv = nn.Sequential(
            block_klass(hidden_size * 2, hidden_size),
            nn.Conv3d(hidden_size, output_channels, 1)
        )

    def forward(self, x, time):
        focus_present_mask = torch.tensor([False]).to(x.device)
        time_rel_pos_bias = self.time_rel_pos_bias(x.shape[2], device=x.device)
        x = self.init_temporal_attn(self.init_conv(x), pos_bias=time_rel_pos_bias)

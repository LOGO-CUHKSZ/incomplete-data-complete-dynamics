...
from typing import Dict, Sequence, Callable, Literal
...
class DM(nn.Module, SDEGenerative):
    ...
    def get_loss(self, batch: Sequence[Tensor]) -> tuple[Dict[str, Tensor], int]:
        ...
        match self.learning.model:
            case 'dsm':
                model_pred: Tensor = self.network(t, xt, mask)
                assert model_pred.shape == noise.shape
                mask = mask.expand_as(x0)
                loss = ((model_pred - x0)[mask]).square().mean()
            case 'ldpv':
                ctx, _, ctx_mask, qry_mask = diffusion_fields(xt, mask, **self.learning)
                model_pred: Tensor = self.network(t, ctx, ctx_mask)
                assert model_pred.shape == noise.shape
                
                qry_mask = qry_mask.expand_as(x0)
                loss = ((model_pred - x0)[qry_mask]).square().mean()
            case _:
                raise ValueError()
        ...
        **kwargs
        ...
        if steps == 1:
            ...
            if sampling is not None and sampling.get('random_context', False):
                ...
                pred = []
                for _ in range(10):
                    ctx, _, ctx_mask, _ = diffusion_fields(
                ...
                match self.learning.model:
                    case 'dsm':
                        model_input = [xt, mask]
                    case 'ldpv':
                        ctx, _, ctx_mask, _ = diffusion_fields(xt, mask, **self.learning)
            ...
            for _ in range(n_expect):
                ...
                context, _, context_mask, _ = diffusion_fields(x0_, mask, **self.learning)
            ...
            for s, t in pbar:
                ...
                context, _, context_mask, _ = diffusion_fields(
            ...
            for _ in range(n_expect):
                random_mask = mask_func(xt)
                context, _, context_mask, _ = diffusion_fields(xt, random_mask, **self.learning)
                model_input = [context, context_mask]
            ...
            context, _, context_mask, _ = diffusion_fields(xt, mask, **self.learning)
            for s, t in pbar:
                model_input = [context, context_mask]
                eps_model = clip(self.pred_noise(s, xt, model_input, **kwargs))
                observed_noise = clip((xt - self.sde.alpha(s) * x0) / self.sde.sigma(s))
